...
def _randomize_prop_colors(prim_path_expr: str) -> int:
    ...
    touched = 0
    if not prim_paths:
        print(f"[render_bench]   (no prims matched {prim_path_expr})")
        return 0
    for prim_path in prim_paths:
        ...
        if not shader_attr:
            if touched == 0:  # log once per expr
                print(f"[render_bench]   no diffuseColor Shader under {prim_path}; children:")
...
class RenderBenchEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
            ...
            ground_rigid_cfg = RigidObjectCfg(
                ...
                spawn=sim_utils.CuboidCfg(
                    ...
                    visual_material=sim_utils.PreviewSurfaceCfg(
                        diffuse_color=self.cfg.flat_ground_color, metallic=0.0
                    ),
        ...
        dome_cfg = sim_utils.DomeLightCfg(
            intensity=self.cfg.dome_light_intensity, color=(0.75, 0.75, 0.75)
        )
        ...
        for prop_name in self.cfg.randomize_prop_colors or []:
            ...
            print(f"[render_bench] randomized {touched} {prop_name} prop colors")
    ...
    def _randomize_dexcube_orientation(self):
        ...
        print(f"[render_bench] randomized DexCube rotation for {self.num_envs} envs")
    ...
    def _lazy_init_motion(self):
        ...
        self._ik_env_phase = torch.arange(self.num_envs, device=self.device) * (
            cycle_total // max(self.num_envs, 1)
        )
        ...
        if (
            ...
            print(
                f"[render_bench] lift_cube_task: sampled {self.num_envs} targets in "
    ...
    def _sin_animate(self, skip: str | None = None):
        ...
        for name, art in self._articulations.items():
            ...
            target = default_pos + self.cfg.joint_animation_amplitude * torch.sin(
                omega * self._anim_t + phase
            )
